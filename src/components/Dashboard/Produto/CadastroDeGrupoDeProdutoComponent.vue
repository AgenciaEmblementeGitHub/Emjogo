            ...
            <div style="margin: 34px 0px; display: flex; flex-direction: row; justify-content: flex-start; flex-wrap: wrap;">
                <div style="display: flex; width: 100%; background-color: #232425; border: 1px solid #00000026;">
                    <ul style="list-style:none; display: flex; height: 40px; align-items: center; box-sizing: border-box; padding: 0px 0px 0px 21px">
                        <li style="margin: 0px 12px 0px 0px; font-size: 14px; padding:0; color: #fefefe; font-weight: 600;">Cadastrar Grupo de Produto </li>
                    </ul>
                </div>
                <fieldset style="width: 100%; min-height: 50%; height: auto; display: flex; justify-content: flex-start; box-sizing: border-box; padding: 21px;">               
                    <div>
                        ...
                        <div>
                            <input type="text" name="description" id="description" placeholder="Descrição" style="margin-right:19px;" class="input-350" v-model="ProductGroupDataModel.ProductGroup.description" :class="{'input-check': ProductGroupDataModel.ProductGroup.description != ''}">
                        </div>                            
                    </div>                   
                    ...
                    <div>
                        ...
                        <div>
                            <input type="text" name="description" id="description" placeholder="Cód" class="input-40" v-model="ProductGroupDataModel.ProductGroup.code" :class="{'input-check': ProductGroupDataModel.ProductGroup.code != ''}">
                        </div>                            
                    </div>                                                                                                                                                                                                                                               
                </fieldset>
                <button class="button back-color-blue" @click="Insert_New_Product_Group()">Cadastrar Grupo de Produto</button>
            </div>        
            ...
            <div style="margin: 34px 0px; display: flex; flex-direction: column; justify-content: flex-start;">
                ...
                <table style="display: flex; width: 100%;">
                    <tbody style="width: 100%;">
                        <tr v-for="pg in this.ProductGroup" :key="pg.id" style="font-size: 13px; width: 100%; height: 50px; display: flex; justify-content: space-between; box-sizing: border-box; border-bottom: 0.09rem dashed rgb(213, 213, 213); align-items: center; border-radius: 3px;">
                            <th style="width:47px;">
                                <div style="text-align: center; width: 40px; height: 13px;border-radius: 4px;background-color: #adff2f5e;padding: 5px;border: 1px solid #0080005e;display: flex;align-items: center;color: #008000b3;">
                                    <p style="margin: 0 auto; font-size: 11px;">{{pg.status == 1 ? "ATIVO" : "DESATIVADO"}}</p>
                                </div>
                            </th>
                            ...
                            <th style="width:114px;">
                                <div style="100%; border: 1px solid #cecece; height: 30px; border-radius: 3px; display: flex; align-items: center;">
                                    ...
                                    <div @click="Delete_Product_Group(pg.id)" style="width: 57px; height: 100%; display: flex; align-items: center; justify-content: center; background-color: #fb0000;color: #fefefe;font-weight: 900;">
                                        <p>X</p>
                                    </div>                                                                      
                                </div>
                            </th>                                                       
...
export default {
    ...
    methods:{
        Insert_New_Product_Group(){
            Service.Insert_New_Product_Group(this.ProductGroupDataModel.ProductGroup).then(()=>{this.GetAll_Product_Group()});
        },      

        GetAll_Product_Group(){
            Service.GetAll_Product_Group().then((success)=>{this.ProductGroup = success.data.Data});
        },  
        ...
        Delete_Product_Group(value){
            Service.Delete_Product_Group(value).then(()=>{this.GetAll_Product_Group()});
        },
