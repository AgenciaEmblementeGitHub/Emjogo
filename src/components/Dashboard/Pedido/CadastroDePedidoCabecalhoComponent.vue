    ...
    <div id="cadastro-de-produto">
        ...
        <div style="margin: 34px 0px; display: flex; flex-direction: row; justify-content: flex-start; flex-wrap: wrap;">
            ...
            <div style="width: 100%;display: flex; justify-content: flex-start; flex-wrap: wrap; box-sizing: border-box; margin-top:21px; margin-left:21px; font-size: 13px;">                                                                                                                                                                                                                                                                                                                                       
                ...
                <div style="padding: 0px; width: 180px; margin-right:21px;">Dt. Entrega</div>
            ...
            <div style="width: 100%; display: flex; justify-content: space-between; flex-wrap: wrap; box-sizing: border-box; margin-left:21px;">
                <div style="width: 100%; display: flex; justify-content: flex-start; align-items:center; box-sizing: border-box;">
                    ...
                    <div style="width: 140px; box-sizing: border-box; display: flex; margin-right: 21px;">
                        <input @change="getFormatDate()" class="input" type="date" name="dataEntrega" id="" style="width: 100%;" v-model="this.nndate"> 
                    </div>            
                    ...
                    <button style="background-color: transparent;border:none; cursor: pointer;" @click="CadastrarPedido()">
...
export default {
    name:'CadastroDePedidoCabecalhoComponent',
    components:{
        LoadingComponent
    },
    setup(){
    ...
    watch:{
        'dataToCreateOrder.orderDtDel':{
            handler(){

                console.log(this.dataToCreateOrder.orderDtDel)
            }
        }
    },
    ...
    data(){
        return {
            ...
            dataToCreateOrder:{
                ...
                custumerID: '',
                custumerNm: '',
                orderDtCre: '00-00-0000',
                orderDtDel: '0000-00-00',
                orderDetails: '',
    ...
    methods:{
        getFormatDate(){
            let dt = this.nndate.split('-');
            this.dataToCreateOrder.orderDtDel = `${dt[2]}/${dt[1]}/${dt[0]}`;
            
            const timeElapsed = Date.now();
            const today = new Date(timeElapsed);
            this.dataToCreateOrder.orderDtCre = today.toLocaleDateString();
        },
        ObterPedidoPorId(id){
        ...
        CadastrarPedido(){
            ...
            */       
            ...
            OService.Insert_New_Order(this.dataToCreateOrder).then(function (STS) {
                _this.isLoading = !_this.isLoading;
                if(STS.status == 500){
                    _this.toast.error("Aconteceu um erro ao cadastar o pedido.");
                    return;
                }
                _this.$router.push(`cadastro-de-pedidos`);
